...
def is_connected(show_status=False):
    ...
    if S.SESSION and S.SESSION.connected:
        ...
        sublime.status_message('Debuggr: Waiting for response from debugger engine.')
    elif show_status:
        sublime.status_message('Debuggr: No Debuggr session running.')
    return False
...
def connection_error(message):
    ...
    """
    sublime.error_message("Please restart Debuggr debugging session.\nDisconnected from Debuggr debugger engine.\n" + message)
    ...
    try:
        ...
        async_session.start()
    # Reset layout
    sublime.active_window().run_command('debuggr_layout')
...
class SocketHandler(threading.Thread):
    ...
    def execute(self, command):
        ...
        self.watch_expression()
        # Set debug layout
        self.run_command('debuggr_layout')
            ...
            if child.tag == dbgp.ELEMENT_BREAKPOINT or child.tag == dbgp.ELEMENT_PATH_BREAKPOINT:
                ...
                if S.BREAKPOINT_RUN is not None and S.BREAKPOINT_RUN['filename'] == filename and S.BREAKPOINT_RUN['lineno'] == lineno:
                    ...
                    if S.BREAKPOINT_RUN['filename'] in S.BREAKPOINT and S.BREAKPOINT_RUN['lineno'] in S.BREAKPOINT[S.BREAKPOINT_RUN['filename']]:
                        self.run_view_command('debuggr_breakpoint', {'rows': [S.BREAKPOINT_RUN['lineno']], 'filename': S.BREAKPOINT_RUN['filename']})
                    S.BREAKPOINT_RUN = None
                ...
                if S.BREAKPOINT_RUN is not None and (S.BREAKPOINT_RUN['filename'] != filename or S.BREAKPOINT_RUN['lineno'] != lineno):
                    self.run_command('debuggr_execute', {'command': 'run'})
                    return
                # Show debug/status output
                self.status_message('Debuggr: Breakpoint')
        ...
        if response.get(dbgp.ATTRIBUTE_STATUS) == dbgp.STATUS_STOPPING or response.get(dbgp.ATTRIBUTE_STATUS) == dbgp.STATUS_STOPPED:
            self.run_command('debuggr_session_stop', {'restart': True})
            self.run_command('debuggr_session_start', {'restart': True})
            self.status_message('Debuggr: Finished executing file on server. Reload page to continue debugging.')
    ...
    def init(self):
        ...
        if get_value(S.KEY_BREAK_ON_START):
            ...
            filename = get_real_path(fileuri)
            # Show debug/status output
            self.status_message('Debuggr: Break on start')
    ...
    def status(self):
        ...
        self.status_message("Debuggr status: " + response.get(dbgp.ATTRIBUTE_REASON) + ' - ' + response.get(dbgp.ATTRIBUTE_STATUS))

...
def load_project_values():
    try:
        ...
        # Use 'debuggr' as key which contains dictionary with project values for package
        S.CONFIG_PROJECT = settings.get(S.KEY_DEBUGGR)
    except:
        pass
...
def get_window_value(key, default_value=None):
    ...
    try:
        ...
        if settings.has(S.KEY_DEBUGGR):
            debuggr = settings.get(S.KEY_DEBUGGR)
            if isinstance(debuggr, dict) and key in debuggr.keys():
                return debuggr[key]
    except:
        pass
    return default_value
...
def set_project_value(key, value=None):
    ...
    if S.KEY_DEBUGGR not in project[S.KEY_SETTINGS].keys() or not isinstance(project[S.KEY_SETTINGS][S.KEY_DEBUGGR], dict):
        project[S.KEY_SETTINGS][S.KEY_DEBUGGR] = {}
    # Update Debuggr settings
    if value is not None:
        project[S.KEY_SETTINGS][S.KEY_DEBUGGR][key] = value
    elif key in project[S.KEY_SETTINGS][S.KEY_DEBUGGR].keys():
        del project[S.KEY_SETTINGS][S.KEY_DEBUGGR][key]
    # Save project data
...
def set_window_value(key, value=None):
    ...
    try:
        ...
        if value is not None:
            debuggr[key] = value
        elif key in debuggr.keys():
            del debuggr[key]
        settings.set(S.KEY_DEBUGGR, debuggr)
    except:
        pass
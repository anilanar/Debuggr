...
def get_real_path(uri, server=False):
    ...
    if isinstance(path_mapping, dict):
        ...
        sublime.set_timeout(lambda: sublime.status_message("Debuggr: No path mapping defined, returning given path."), 100)
...
def get_region_icon(icon):
    ...
    if S.PACKAGE_FOLDER is not None:
        ...
        if sublime.version() == '' or int(sublime.version()) > 3000:
            # ST3: Packages/Debuggr Client/icons/breakpoint_enabled.png
            icon_path = "Packages/" + current_package + '/icons/{0}.png'
        else:
            # ST2: ../Debuggr Client/icons/breakpoint_enabled
...
def launch_browser():
    ...
    if not url:
        sublime.set_timeout(lambda: sublime.status_message('Debuggr: No URL defined in (project) settings file.'), 100)
        return
    ...
    if S.SESSION and (S.SESSION.listening or not S.SESSION.connected):
        webbrowser.open(url + operator + 'DEBUGGR_SESSION_START=' + ide_key)
    # Stop debug session
    else:
        # Check if we should execute script
        if get_value(S.KEY_BROWSER_NO_EXECUTE):
            # Without executing script
            webbrowser.open(url + operator + 'DEBUGGR_SESSION_STOP_NO_EXEC=' + ide_key)
        else:
            # Run script normally
            webbrowser.open(url + operator + 'DEBUGGR_SESSION_STOP=' + ide_key)

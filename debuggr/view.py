...
TITLE_WINDOW_BREAKPOINT = "Debuggr Breakpoint"
TITLE_WINDOW_CONTEXT = "Debuggr Context"
TITLE_WINDOW_STACK = "Debuggr Stack"
TITLE_WINDOW_WATCH = "Debuggr Watch"
...
def close_debug_windows():
    ...
    window.run_command('hide_panel', {"panel": 'output.debuggr'})
...
def show_content(data, content=None):
    ...
    view.settings().set('syntax', 'Packages/' + package + '/Debuggr.tmLanguage')

    # Set content for view and fold all indendation blocks
    view.run_command('debuggr_view_update', {'data': content, 'readonly': True})
            ...
            if point.size() == 0 and sublime.score_selector(view.scope_name(point.a), 'variable'):
                ...
                if match:
                    ...
                    if variable:
                        ...
                        window = sublime.active_window()
                        panel = window.get_output_panel('debuggr')
                        panel.run_command("debuggr_view_update", {'data' : data} )
                        panel.run_command('set_setting', {"setting": 'word_wrap', "value": True})
                        window.run_command('show_panel', {"panel": 'output.debuggr'})
        except:
            pass
...
def show_panel_content(content):
    ...
    try:
        window = sublime.active_window()
        panel = window.get_output_panel('debuggr')
        panel.run_command('debuggr_view_update', {'data': content})
        panel.run_command('set_setting', {"setting": 'word_wrap', "value": True})
        window.run_command('show_panel', {'panel': 'output.debuggr'})
    except:
        print(content)
...
def toggle_breakpoint(view):
    try:
        ...
        if point.size() == 3 and sublime.score_selector(view.scope_name(point.a), 'debuggr.output.breakpoint.line'):
            ...
            if match and match.group('line_number'):
                ...
                breakpoint_file = view.find_by_selector('debuggr.output.breakpoint.file')
                ...
                if file_match and file_match.group('filename'):
                    ...
                    if enabled is None:
                        return
                    sublime.active_window().run_command('debuggr_breakpoint', {"enabled": enabled, "rows": [line_number], "filename": filename})
        # Check if selected point uses breakpoint file scope
        elif point.size() > 3 and sublime.score_selector(view.scope_name(point.a), 'debuggr.output.breakpoint.file'):
...
def toggle_stack(view):
    try:
        ...
        if point.size() > 3 and sublime.score_selector(view.scope_name(point.a), 'debuggr.output.stack.entry'):
...
def toggle_watch(view):
    ...
    try:
        ...
        if point.size() == 3 and sublime.score_selector(view.scope_name(point.a), 'debuggr.output.watch.entry'):
            ...
            if match and (match.group('enabled') or match.group('disabled')):
                ...
                watch = view.find_by_selector('debuggr.output.watch.entry')
                watch_index = 0
                for entry in watch:
                    # Stop searching if we have passed selected breakpoint
                    if entry > point:
                ...
                sublime.active_window().run_command('debuggr_watch', {"update": True})
    except:
        pass